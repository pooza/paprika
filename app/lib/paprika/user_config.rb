module Paprika
  class UserConfig
    ...
    def initialize(account)
      ...
      if account.is_a?(Enumerable) && (token = account['/paprika/token'])
    ...
    def token
      return (self['/paprika/token'] || self['/webhook/token']).decrypt
    rescue
      return self['/paprika/token'] || self['/webhook/token']
    end
    ...
    def handle_user_tags(values)
      ...
      if minutes = flatten['/tagging/minutes']
        Sidekiq.set_schedule("user_tag_initialize_#{@account.username}", {
          ...
          class: 'Paprika::UserTagInitializeWorker',
